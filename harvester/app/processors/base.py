...
class ProcessorRegistry(metaclass=Singleton):
    ...
    def __init__(self):
        for cls in self.all_subclasses(EventProcessor):
            key = '{}-{}'.format(cls.module_id, cls.event_id).lower()

            if key not in self.registry['event']:
                self.registry['event'][key] = []

            self.registry['event']['{}-{}'.format(cls.module_id, cls.event_id).lower()].append(cls)

        for cls in self.all_subclasses(ExtrinsicProcessor):
            key = '{}-{}'.format(cls.module_id, cls.call_id).lower()
    ...
    def get_event_processors(self, module_id, event_id):
        return self.registry['event'].get('{}-{}'.format(module_id, event_id).lower(), [])

    def get_extrinsic_processors(self, module_id, call_id):
        return self.registry['extrinsic'].get('{}-{}'.format(module_id, call_id).lower(), [])
