...
class Event(BaseModel):
    ...
    contract_event = sa.Column(sa.JSON())

    contract_instance_address = sa.Column(sa.String(48))
    
    def serialize_id(self):
        return '{}-{}'.format(self.block_id, self.event_idx)

class AccountPrivate(BaseModel):
    __tablename__ = 'account_private'

    id = sa.Column(sa.Integer(), primary_key=True, index=True)
    account = sa.Column(sa.String(128), nullable=True, index=True)
    is_private = sa.Column(sa.Integer(), nullable=True)
...
class Extrinsic(BaseModel):
    ...
    contract_message = sa.Column(sa.JSON())
    to_address = sa.Column(sa.String(48), index=True)
...
class ContractInstance(BaseModel):
    __tablename__ = 'data_contract_instance'

    address = sa.Column(sa.String(48), primary_key=True)
    code_hash = sa.Column(sa.String(64), index=True)
    owner = sa.Column(sa.String(48))
    name = sa.Column(sa.String(128))
    symbol = sa.Column(sa.String(128))
    created_at_block = sa.Column(sa.Integer(), nullable=False)
    created_at_extrinsic = sa.Column(sa.Integer())
    created_at_event = sa.Column(sa.Integer())
...
class ContractInstanceExtrinsic(BaseModel):
    __tablename__ = 'data_contract_extrinsic'

    block_id = sa.Column(sa.Integer(), nullable=False, primary_key=True)
    extrinsic_idx = sa.Column(sa.Integer(), nullable=False, primary_key=True)
    from_address = sa.Column(sa.String(48), nullable=False, index=True)
    to_address = sa.Column(sa.String(48), index=True)
    contract_address = sa.Column(sa.String(48), index=True)
    message_name = sa.Column(sa.String(256), index=True)
    message_args = sa.Column(sa.JSON(), default=None, server_default=None, nullable=True)
